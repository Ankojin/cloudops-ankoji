...
foreach ($vm in $vmsToClone) {
    ...
    if (-not $snapshotOS) {
        ...
        $snapshotOS = New-AzSnapshot -ResourceGroupName $targetResourceGroup -SnapshotName $snapshotOSName `
            -Snapshot (New-AzSnapshotConfig -SourceResourceId $osDisk.Id -Location $targetLocation -CreateOption Copy)
    } else {
    ...
    if (-not $newOSDisk) {
        ...
        $newOSDisk = New-AzDisk -ResourceGroupName $targetResourceGroup -DiskName $newOSDiskName `
            -Disk (New-AzDiskConfig -Location $targetLocation -CreateOption Copy -SourceResourceId $snapshotOS.Id)
    } else {
    ...
    foreach ($dataDisk in $sourceVM.StorageProfile.DataDisks) {
        ...
        if (-not $snapshot) {
            ...
            $snapshot = New-AzSnapshot -ResourceGroupName $targetResourceGroup -SnapshotName $snapshotName `
                -Snapshot (New-AzSnapshotConfig -SourceResourceId $disk.Id -Location $targetLocation -CreateOption Copy)
        } else {
        ...
        if (-not $clonedDisk) {
            ...
            $clonedDisk = New-AzDisk -ResourceGroupName $targetResourceGroup -DiskName $clonedDiskName `
                -Disk (New-AzDiskConfig -Location $targetLocation -CreateOption Copy -SourceResourceId $snapshot.Id)
        } else {
