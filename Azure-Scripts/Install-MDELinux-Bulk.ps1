...
Write-Host "`n=== Bulk Install: MDE.Linux Extension ===" -ForegroundColor Cyan
...
foreach ($sub in $SubscriptionIds) {
    ...
    foreach ($vm in $vms) {
        ...
        $mde = $ext | Where-Object { $_.ExtensionType -eq "MDE.Linux" }
        ...
        Write-Host "Installing MDE.Linux on $($vm.Name)..." -ForegroundColor Cyan
...
$csv = "MDE-Linux-Install-Report-$(Get-Date -Format yyyyMMddHHmm).csv"
