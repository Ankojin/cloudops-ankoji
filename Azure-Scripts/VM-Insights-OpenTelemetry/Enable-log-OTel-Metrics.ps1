<#
.SYNOPSIS
    Enable Azure Monitor Agent (AMA), Classic VM Insights Log-based
    metrics, existing performance-counter metrics and OpenTelemetry
    metrics on Azure VMs.
...
.DESCRIPTION
    This script performs the following:
      ...
      9. Installs AMA if missing.
     10. Enables System Assigned Managed Identity if required.
     11. Skips stopped VMs.
     12. Skips deallocated VMs.
     13. Skips VMs whose state isn't confirmed as Running.
     14. Skips ARO-INFRA-* resource groups.
     15. Creates the VM -> shared DCR association when missing.
     16. Corrects the VM association if it points to another DCR.
     17. Verifies the final DCR configuration.
     18. Exports detailed results to CSV.
...
.PARAMETER AmaWaitSeconds
    Maximum time to wait for AMA provisioning.
...
param(
    ...
    [string]$DcrResourceId = "/subscriptions/43cc4f11-ffb1-4a0d-8420-0ba3746b4248/resourcegroups/bab-dev-wrkspace-swec-rg-01/providers/microsoft.insights/datacollectionrules/msvmi-bab-dev-vm-monitoring-dcr",
...
$ExpectedWorkspaceResourceId =
    "/subscriptions/43cc4f11-ffb1-4a0d-8420-0ba3746b4248/resourceGroups/bab-dev-wrkspace-swec-rg-01/providers/Microsoft.OperationalInsights/workspaces/bab-dev-all-wrkspc-swec-01"
...
"@
    ...
    foreach ($vm in $allVMs) {
        ...
        if ($WhatIfPreference) {
            ...
            $result.Message =
                "AMA ready; DCR association ready; Classic VM Insights and OpenTelemetry configuration ready"
